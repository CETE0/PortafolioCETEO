...
export default function ShooterLegacy() {
  ...
  useEffect(() => {
    ...
    return () => { preloadedImagesRef.current.clear(); };
      ...
      setLegFrame(prev => {
        if (prev >= LEG_FRAMES - 1) {
          ...
          setTimeout(() => {
            setCanKick(true);
            if (gameStarted) {
              if (shouldIncreaseScore) setScore(prev => prev + 1);
              getRandomImage();
  ...
  useEffect(() => {
    if (isFalling) {
      const fallInterval = setInterval(() => {
        setFallFrame(prev => {
          if (prev >= FALL_FRAMES - 1) { clearInterval(fallInterval); return prev; }
          return prev + 1;
        });
      }, FRAME_DURATION);
      ...
      <div className="relative min-h-screen flex flex-col items-center justify-center bg-white">
        <div className="relative w-full max-w-4xl aspect-[4/3] flex items-center justify-center">
          <div className="text-center">
            <div className="text-black font-['Press_Start_2P'] text-lg mb-4">{t('home.loadingAssets')}</div>
            <div className="w-64 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-red-600 transition-all duration-300" style={{ width: `${loadingProgress}%` }} />
            </div>
            <div className="text-black font-['Press_Start_2P'] text-xs mt-2">{Math.round(loadingProgress)}%</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center bg-white">
      <div className={`relative w-full max-w-4xl aspect-[4/3] perspective-1000 ${!gameStarted && !showStart ? 'pointer-events-none' : ''}`} onClick={handleGameAreaClick} style={{ cursor: showStart || gameStarted ? 'pointer' : 'default' }}>
        <div className="absolute inset-0">
          <AnimatePresence mode="wait">
            {gameStarted && backgroundUrl ? (
              <BackgroundImage key={backgroundUrl} url={backgroundUrl} shouldAnimate={gameStarted} isGameImage={true} isHit={isHit} />
            ) : (
              <motion.div key="initial-bg" className="absolute inset-0" initial={false} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                <AnimationImage src={ANIMATION_ASSETS.background} alt={t('home.backgroundAlt')} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="absolute inset-0 z-10">
          {!gameStarted && assetsLoaded && (
            <>
              <div className="absolute inset-0">
                <AnimationImage src={ANIMATION_ASSETS.plinth[isFalling ? Math.min(fallFrame, FALL_FRAMES - 1) : 0]} alt="Plinth" />
              </div>
              <div className="absolute inset-0">
                <AnimationImage src={ANIMATION_ASSETS.object[isFalling ? Math.min(fallFrame, FALL_FRAMES - 1) : 0]} alt="Object" />
              </div>
            </>
          )}
          {isLegAnimating && assetsLoaded && (
            <div className={`absolute inset-0 ${isFlipped ? 'scale-x-[-1]' : ''}`}>
              <AnimationImage src={ANIMATION_ASSETS.leg[Math.min(legFrame, LEG_FRAMES - 1)]} alt="Leg" />
            </div>
          )}
        </div>
        ...
        <div className="absolute inset-0 z-20 pointer-events-none">
          {showStart && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span className={`${lang === 'es' ? 'text-4xl md:text-5xl' : 'text-6xl'} font-['Press_Start_2P'] text-red-600`}>{t('home.start')}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="w-full flex justify-center gap-4 font-['Press_Start_2P'] text-sm mt-4">
        {!gameStarted && (<span className="text-red-600">{t('home.tapToStart')}</span>)}
        {gameStarted && (<span className="text-red-600">{t('home.score', { score })}</span>)}
      </div>
    </div>
  );
}
